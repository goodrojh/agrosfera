...
- **Предприятия.** Бот принимает цены только от подтверждённых. Новый пользователь проходит анкету: название, ИНН (проверяется контрольная сумма), регион склада, ФИО и должность, телефон (кнопкой «Отправить номер» — только свой). Анкета приходит в админ-чат со ссылкой на проверку ИНН и кнопками «Подтвердить / Отклонить». Если ИНН уже есть в базе, новый сотрудник привязывается к существующему предприятию. Команда `/pending` в админ-чате — список анкет на проверке.
- **Коды приглашения** (`npm run admin -- add ...`) по-прежнему работают: предприятие, заведённое вручную, подключается без анкеты.
- **Заявки покупателей с сайта** попадают в стакан только после кнопки «В стакан» в админ-чате.
- Узнать ID чата для `ADMIN_CHAT_ID`: написать боту `/id`.
